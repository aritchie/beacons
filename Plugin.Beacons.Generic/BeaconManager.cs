...
namespace Plugin.Beacons
{
    public class BeaconManager : IBeaconManager
    {
        ...
        readonly BeaconSettings settings;
         ...
         */
        public BeaconManager(BeaconSettings settings = null, IAdapter adapter = null)
        {
            this.settings = settings ?? BeaconSettings.Current;
            this.adapter = adapter ?? CrossBleAdapter.Current;
            this.regionStates = new Dictionary<string, BeaconRegionStatus>();

            foreach (var region in this.settings.MonitorRegions)
            {
        ...
        {
            return Task.FromResult(this.adapter.Status == AdapterStatus.PoweredOn);
        }
        ...
        public IObservable<RangedBeaconList> WhenBeaconsRanged(params BeaconRegion[] regions)
        {
            ...
            return Observable.Create<RangedBeaconList>(ob => this
                .Scan()
